...
main() {
  ...
  readarray -t valid_timezones <<<"$(cat $CURRENT_DIR/../configs/valid_timezones.txt)"
  ...
  [ "$CONFIGURE_LETSENCRYPT" == true ] || [ "$ASSUME_SSL" == true ] && bash "$CURRENT_DIR/../lib/verify-fqdn.sh" "$FQDN"
